...
// let diceNamber = Math.trunc(Math.random() * 6) + 1; //андомная генерация чисел от 1 до 12, Math.trunc используется что бы выкинуть десятичную часть
// console.log(`Dise namber before loop: ${diceNamber}`);

// while (diceNamber !== 6){
//    console.log(diceNamber);
//       diceNamber = Math.trunc(Math.random() * 6) + 1;
//       console.log(`Dise namber after random criation inside the  loop: ${diceNamber}`);
// }
...
/*Давайте улучшим калькулятор подсказок из предыдущей задачи, используя циклы.
Ваши задачи:
1. Создайте массив счетов, содержащий все 10 значений тестовых счетов.
2. Создайте пустые массивы для подсказок и итогов (tips and totals)
3. Используйте функцию calculateTips(), которую мы писали ранее, для вычисления чаевых и общих значений (счет + чаевые) для каждого значения счета в массиве счетов. Используйте цикл for для выполнения 10 вычислений.
Тестовые данные: 31, 95, 276, 540, 27, 205, 11, 1180, 96, 57.
Вызовите calculateTips() в цикле и используйте метод push() для добавления значений в массивы tips и totals.
4. Напишите функцию CalculateAverage(), которая принимает массив с именем arr в качестве параметра. Эта функция вычисляет среднее значение всех чисел в заданном массиве. Это трудная задача! Вот как это решить:
4.1. Сначала вам нужно будет добавить все значения в массив. Чтобы выполнить сложение, начните с создания переменной “sum”, которая начинается с 0. Затем выполните цикл по массиву, используя цикл for. На каждой итерации добавляйте текущее значение к переменной sum. Таким образом, к концу цикла все значения будут суммированы вместе.
4.2. Чтобы вычислить среднее значение, разделите сумму, которую вы рассчитали ранее, на длину массива (потому что это количество элементов).
4.3. Вызовите CalculateAverage() с массивом итогов.*/


//Решение

// const calculateTips = bill => bill < 20 ? bill * 11 : bill * 40;
